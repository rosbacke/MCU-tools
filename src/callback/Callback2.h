 ...
 * - Call a free function of signature  R(*)(T*, Args...); (T is some arbitrary type).
 * - Call a free function of signature void(*)(void*, Args...); (function can be a function pointer)
 *
...
class Callback2<R(Args...)>
{
	...
	template<class T, void (T::*memFkn)(Args...)>
...
public:
	 ...
	 */
	template<class Tptr, void (*fkn)(Tptr, Args... args)>
 ...
 *
 * @param fknType Template parameter for the function signature in std::function style.
...
(Callback2<fknType>::makeMemberCB<std::remove_reference<decltype(object)>::type, &memFkn>(object))
...
(Callback2<fknType>::makeFreeCB<std::remove_reference<decltype(ptr)>::type, fkn>(ptr))
