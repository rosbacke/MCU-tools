 ...
 * vtable.
 *
 * Main intent is to replace explicit use of function callbacks with void pointers
 ...
 * - Call a free function of signature void(*)(T*, int); (T is some arbitrary type).
 * - Call a free function of signature void(*)(void*, int); (function can be a function pointer)
 *
 ...
 * int val = 0;
 * cb(val);
 * For free functions, the supplied void pointer get the value from the call setup.
 */
class Callback
{
	...
	inline static void doMemberCB(void* o, int val) __attribute__((always_inline));

	template<class Tptr, void (freeFkn)(Tptr, int)>
	inline static void doFreeCB(void* o, int val) __attribute__((always_inline));
...
public:
	...
	/**
	 * Create a callback to a free function with a void pointer, removing the need for an adapter function.
	 */
	template<CB fkn>
	...
	/**
	 * Create a callback using a run-time variable fkn pointer using voids. No adapter function is used.
	 */
...
template<class T, void (T::*memFkn)(int)>
void Callback::doMemberCB(void* o, int val)
{
...
template<class Tptr, void (freeFkn)(Tptr, int)>
void Callback::doFreeCB(void* o, int val)
{
...
(Callback::makeMemberCB<std::remove_reference<decltype(object)>::type, &memFkn>(object))
